...
<script>
  ...
  export default {
    ...
    async onLoad() {
      ...
      this.form_libraries = this.$store.state.perfection.chIdiom_libraries.map(item => {
        return item.id
      })
    },
    onBackPress() {
    ...
    methods: {
      async fetchData() {
        await this.$http.get('/perfection/chIdiom_libraries/')
          .then(resp => {
      ...
      setLibraries() {
        ...
        this.$http.post('/perfection/student/set_chIdioms_library/', {
            chIdiom_libraries: this.form_libraries
          })
          .then(resp => {
            this.changed = false
