<template>
  <view>
    <u-navbar title="重发激活邮件"></u-navbar>
    <u-text type="info" text="我们将会给您发送一封邮件" align="center"></u-text>
    <u-text type="info" text="您需要点击邮件内的链接来激活您的账号" align="center"></u-text>
  ...
  export default {
    ...
    methods: {
      send() {
        ...
        this.loading = true
        this.wrong = false
        this.$http.post('/users/resend_activation/', {
            email: this.email
          })
          .then(resp => {
            this.modal = {
              show: true,
              title: '发送成功',
              content: '请查看您的邮箱并完成激活，如果没有收到请检查邮箱地址是否输入错误或查找您的垃圾箱'
            }
            this.disabled = true
            this.loading = false
          })
