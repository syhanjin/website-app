<template>
  <view>
    ...
    <u-loading-page loading-text="正在加载单词内容..." :loading="loading"></u-loading-page>
...
<script>
  ...
  export default {
    ...
    methods: {
      next(known) {
        if (this.current < this.review.length) {
          this.submit.review[this.review[this.current].word] = known
        } else {
          this.submit.addition[this.addition[this.current - this.review.length].word] = known
        }
        this.current++
      ...
      async fetchData() {
        ...
        await this.$http.get('/perfection/words/' + this.wp_id + '/review/')
          .then(resp => {
            // console.log(resp)
            this.review = this.$u.randomArray(resp.data.review)
            // console.log(this.review)
          })
        await this.$http.get('/perfection/words/' + this.wp_id + '/addition/')
          .then(resp => {
    ...
    computed: {
      ...
      k_() {
        return this.word_.word
      },
      v_() {
        return this.word_.chinese
      },
    }
  }
</script>
